...
router.put("/:Id_Recetas_Dia", (req, res) => {
  ...
  if (error) {
    ...
    const query = `
      UPDATE recetas_dia
      SET Id_Receta_Cena = ?, Id_Usuario_Modif = ?, Fecha_Modif = NOW()
    ...
    db.query(query, [id_receta, Id_Usuario_Alta, Id_Recetas_Dia], (err, result) => {
      ...
      res.json({ message: "Cena actualizado con éxito" });
    });
  });
...
if (error) {
  ...
  return res.status(400).json({ error: error.details[0].message });
    ...
    const query = `
      UPDATE recetas_dia
      SET Id_Receta_Cena = NULL, Id_Usuario_Modif = ?, Fecha_Modif = NOW()
      WHERE Id_Recetas_Dia = ?
    `;
    ...
    db.query(query, [Id_Usuario_Modif, Id_Recetas_Dia], (err, result) => {
      ...
      res.json({ message: "Cena eliminado" });
    });
  });
