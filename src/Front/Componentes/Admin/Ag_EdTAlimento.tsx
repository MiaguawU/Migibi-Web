...
interface TAlimentoModalProps {
  visible: boolean;
  onClose: () => void;
  id_unidad: number | null; 
}

const TAlimentoModal: React.FC<TAlimentoModalProps> = ({ visible, onClose, id_unidad }) => {
  ...
  const obtenerUnidad = async (alimentoId: number, adminId: number) => {
    ...
    try {
      const response = await axios.get(`${PUERTO}/tipoA/${id_unidad}`);
      ...
      if (Array.isArray(unidadDataArray) && unidadDataArray.length > 0) {
        ...
        form.setFieldsValue({
          Tipo_Alimento: unidad.Tipo_Alimento,
        });
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      if (id_unidad !== null) {
        ...
        await axios.put(`${PUERTO}/tipoA/${id_unidad}`, payload); // Send JSON directly
        message.success('Unidad actualizado correctamente.');
      } else {
        await axios.post(`${PUERTO}/tipoA`, payload); // Send JSON directly, adminId in URL
  ...
  return (
    <Modal
      title={<span style={{ fontSize: '20px', fontWeight: 'bold' }}>{id_unidad !== null ? "Editar tipo alimento" : "Agregar tipo alimento"}</span>}
      open={visible}
      onCancel={onClose}
      footer={[
        ...
        <Button key="submit" type="primary" loading={loading} onClick={handleFormSubmit} style={{ fontSize: '16px' }}>
          {id_unidad !== null ? "Guardar Cambios" : "Agregar Tipo alimento"}
        </Button>,
      ]}
    >
      {modalLoading ? (
        ...
        >
          <Form.Item
            name="Tipo_Alimento"
            label={<span style={{ fontSize: '16px' }}>Nombre del tipo alimento</span>}
            rules={[{ required: true, message: 'Por favor ingresa el nombre del alimento!' }]}
          >
            <Input placeholder="Ej. fruta" style={{ fontSize: '16px' }} />
          </Form.Item>
