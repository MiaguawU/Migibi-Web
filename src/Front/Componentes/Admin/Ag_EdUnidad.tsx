...
interface UnidadModalProps {
  visible: boolean;
  onClose: () => void;
  id_unidad: number | null; // El ID del alimento a editar. Si es null, es un nuevo alimento.
}

const UnidadModal: React.FC<UnidadModalProps> = ({ visible, onClose, id_unidad }) => {
  ...
  const obtenerUnidad = async (alimentoId: number, adminId: number) => {
    ...
    try {
      ...
      const response = await axios.get(`${PUERTO}/unidad/${alimentoId}`);
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      if (id_unidad !== null) {
        ...
        await axios.put(`${PUERTO}/unidad/${id_unidad}`, Upayload); // Send JSON directly
        message.success('Unidad actualizado correctamente.');
      } else {
        // Modo Creación:
        console.log("Payload enviado (POST):", payload);
        await axios.post(`${PUERTO}/unidad`, payload); // Send JSON directly, adminId in URL
  ...
  return (
    <Modal
      title={<span style={{ fontSize: '20px', fontWeight: 'bold' }}>{id_unidad !== null ? "Editar Unidad" : "Agregar Unidad"}</span>}
      open={visible}
      onCancel={onClose}
      footer={[
        ...
        <Button key="submit" type="primary" loading={loading} onClick={handleFormSubmit} style={{ fontSize: '16px' }}>
          {id_unidad !== null ? "Guardar Cambios" : "Agregar Unidad"}
        </Button>,
      ]}
    >
      {modalLoading ? (
        ...
        >
          <Form.Item
            name="Unidad_Medida"
            label={<span style={{ fontSize: '16px' }}>Nombre de la unidad</span>}
            rules={[{ required: true, message: 'Por favor ingresa el nombre del alimento!' }]}
          >
            <Input placeholder="Ej. gramos" style={{ fontSize: '16px' }} />
          </Form.Item>

          <Form.Item
            name="Abreviatura"
            label={<span style={{ fontSize: '16px' }}>Abreviatura</span>}
            rules={[{ required: true, message: 'Por favor ingresa el nombre del alimento!' }]}
          >
            <Input placeholder="Ej. gr" style={{ fontSize: '16px' }} />
          </Form.Item>
