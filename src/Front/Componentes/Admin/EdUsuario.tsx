...
const EditUserModal: React.FC<EditUserModalProps> = ({ visible, onClose, id_user }) => {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      // Reverting to your original URL based on the log: /us_adm/unic/adminId/userId
      const response = await axios.get<UserData[]>(`${PUERTO}/us_adm/unic/${adminId}/${id_user}`);
        const userData = response.data[0]; // accede al primer objeto del array

      const rolParsed = parseInt(userData.Id_Rol as string, 10);
        form.resetFields();
        form.setFieldsValue({
        Nombre_Usuario: userData.Nombre_Usuario,
        Cohabitantes: userData.Cohabitantes,
        Email: userData.Email,
        Id_Rol: isNaN(rolParsed) ? undefined : rolParsed,
        });
      ...
      if (userData.foto_perfil) {
        if (userData.foto_perfil.startsWith('http://') || userData.foto_perfil.startsWith('https://')) {
          ...
          setPreviewImage(`${PUERTO}/${userData.foto_perfil}`);
        }
      } else {
        setPreviewImage(undefined);
      }
      setFileImage(null);
      message.success('Datos de usuario cargados correctamente.');
    } catch (error: any) {
      console.error('Error al cargar datos del usuario:', error.response?.data || error.message);
      message.error(`Error al cargar los datos del usuario: ${error.response?.data?.error || error.message}.`);
      onClose();
    } finally {
      setModalLoading(false);
    }
  ...
  useEffect(() => {
    if (visible && id_user !== null) {
        obtenerRoles();
      fetchUserData();
    } else if (visible && id_user === null) {
      form.resetFields();
      form.setFieldsValue({ Cohabitantes: 1, Id_Rol: 2 }); // Default role for new users
      setPreviewImage(undefined);
      setFileImage(null);
    }
  }, [visible, id_user, form]);
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      } else if (id_user !== null && !previewImage) {
      ...
      if (id_user !== null) {
        // Use the original PUT endpoint based on your API structure
        await axios.put(`${PUERTO}/us_adm/${id_user}/${adminId}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        message.success('Usuario actualizado correctamente.');
      } else {
        // Use the original POST endpoint
        await axios.post(`${PUERTO}/api/users?adminId=${adminId}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
  ...
  return (
    <Modal
      title={<span style={{ fontSize: '20px', fontWeight: 'bold' }}>{id_user !== null ? "Editar Usuario" : "Crear Usuario"}</span>}
      open={visible}
      onCancel={onClose}
      footer={[
      ...
      {modalLoading ? (
        ...
        >
          ...
          <Form.Item
            ...
            rules={id_user === null ? [{ required: true, message: 'Por favor ingresa una contraseña!' }] : []}
          >
            <Input.Password
              placeholder={id_user !== null ? "Deja vacío para no cambiar" : "Ingresa la contraseña"}
          ...
          <Form.Item
            ...
            rules={[{ type: 'email', message: 'El formato del email no es válido.' }]}
          >
